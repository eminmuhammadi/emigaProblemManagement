<?php
    ...
    $total_p = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='P' ");
    $total_p = mysqli_num_rows($total_p);

    $total_d = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='D' ");
    $total_d = mysqli_num_rows($total_d);

    $total_c = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='C' ");
    $total_c = mysqli_num_rows($total_c);

    $total_v = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='V' ");
    $total_v = mysqli_num_rows($total_v);


    /*Today Charts Statics*/

    $today_p = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='P' && (reg_date > CURDATE() || reg_date = CURDATE())");
    $today_p = mysqli_num_rows($today_p);

    $today_d = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='D' && (reg_date > CURDATE() || reg_date = CURDATE())");
    $today_d = mysqli_num_rows($today_d);

    $today_c = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='C' && (reg_date > CURDATE() || reg_date = CURDATE())");
    $today_c = mysqli_num_rows($today_c);

    $today_v = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='V' && (reg_date > CURDATE() || reg_date = CURDATE())");
    $today_v = mysqli_num_rows($today_v);


    /* All Statics Compare*/

    $all_problems = mysqli_query($connect,"SELECT problem_id FROM posts");
    $all_problems = mysqli_num_rows($all_problems);

    $all_problems_today = mysqli_query($connect,"SELECT problem_id FROM posts WHERE (reg_date > CURDATE() || reg_date = CURDATE())");
    $all_problems_today = mysqli_num_rows($all_problems_today);

    $all_problems_month = mysqli_query($connect,"SELECT problem_id FROM posts WHERE ((reg_date > curdate() - interval 30 day)||(reg_date > curdate() - interval 30 day))");
...
<div class="col-md-12 grid-margin">
    ...
    <div class="col-12 grid-margin pt-2"><i class="icon-alert"> </i> Statistika <b><mark>bütün</mark></b> şöbələr üçün seçilmişdir.</div>        
    </div>
...
<?php           
    ...
    $select_departments ="SELECT problem_title , problem_description , problem_status_description FROM posts ORDER BY problem_id DESC ";
    $result = mysqli_query($connect, $select_departments);
