<?php
    ...
    $total_p = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='P' && user_id='$user_id' ");
    $total_p = mysqli_num_rows($total_p);

    $total_d = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='D' && user_id='$user_id' ");
    $total_d = mysqli_num_rows($total_d);

    $total_c = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='C' && user_id='$user_id' ");
    $total_c = mysqli_num_rows($total_c);

    $total_v = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='V' && user_id='$user_id' ");
    $total_v = mysqli_num_rows($total_v);


    /*Today Charts Statics*/

    $today_p = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='P'  && (reg_date > CURDATE() || reg_date = CURDATE()) && user_id='$user_id' ");
    $today_p = mysqli_num_rows($today_p);

    $today_d = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='D'  && (reg_date > CURDATE() || reg_date = CURDATE()) && user_id='$user_id' ");
    $today_d = mysqli_num_rows($today_d);

    $today_c = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='C'  && (reg_date > CURDATE() || reg_date = CURDATE()) && user_id='$user_id' ");
    $today_c = mysqli_num_rows($today_c);

    $today_v = mysqli_query($connect,"SELECT problem_id FROM posts WHERE problem_status='V'  && (reg_date > CURDATE() || reg_date = CURDATE()) && user_id='$user_id' ");
    $today_v = mysqli_num_rows($today_v);


    /* All Statics Compare*/

    $all_problems = mysqli_query($connect,"SELECT problem_id FROM posts WHERE user_id='$user_id' ");
    $all_problems = mysqli_num_rows($all_problems);

    $all_problems_today = mysqli_query($connect,"SELECT problem_id FROM posts WHERE (reg_date > CURDATE() || reg_date = CURDATE()) && user_id='$user_id' ");
    $all_problems_today = mysqli_num_rows($all_problems_today);

    $all_problems_month = mysqli_query($connect,"SELECT problem_id FROM posts WHERE ((reg_date > curdate() - interval 30 day)||(reg_date > curdate() - interval 30 day)) && user_id='$user_id' ");
...
<div class="col-md-12 grid-margin">
    ...
    <div class="col-12 grid-margin pt-2"><i class="icon-alert"> </i> Aşağıdakı statistika ancaq <b><mark>sənə</mark></b> addir.</div>        
    </div>
             ...
             <div class="card-body p-0">
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-6 col-lg-3">
                      <div class="d-flex justify-content-between border-right card-statistics-item">
                        <div>
                          <h1><?php echo emigaStats($total_d);?></h1>
                          <p class="text-muted mb-0">Həll edilən problemlər</p>
                        </div>
...
<?php           
    ...
    $select_departments ="SELECT problem_title , problem_description , problem_status_description FROM posts WHERE user_id='$user_id' ORDER BY problem_id DESC ";
    $result = mysqli_query($connect, $select_departments);
